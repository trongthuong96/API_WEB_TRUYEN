...
namespace API.Controllers
{
    ...
    public class ChapterController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        //[ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public IActionResult CreateChapter([FromBody] IList<ChapterCreateDto> chapterCreateDtoList)
        {

            // check info null
            if (chapterCreateDtoList == null)
            {
                return BadRequest(ModelState);
            }

            foreach(ChapterCreateDto chapterCreateDto in chapterCreateDtoList)
            {
                // check story
                if (!_storyRepository.StoryExists(chapterCreateDto.StoryId)) //true
                {
